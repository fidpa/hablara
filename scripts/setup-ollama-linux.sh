...
# Returns XDG-aware Ollama data path (respects XDG_DATA_HOME if set and absolute)
...
get_free_space_gb() {
  ...
  if command_exists df; then
    free_space=$(df -BG "${check_path}" 2>/dev/null | tail -1 | awk '{print $4}' || true)
...
run_status_check() {
  ...
  if command_exists ollama; then
    ...
    if [[ "$current_version" != "unbekannt" ]] && ! version_gte "$current_version" "$MIN_OLLAMA_VERSION"; then
      echo -e "    ${COLOR_YELLOW}↳ Update empfohlen (mindestens v${MIN_OLLAMA_VERSION}): curl -fsSL https://ollama.com/install.sh | sh${COLOR_RESET}"
    fi
  else
  ...
  case "$gpu_info" in
    nvidia:*)    echo -e "  ${COLOR_GREEN}✓${COLOR_RESET} GPU: NVIDIA ${gpu_info#nvidia:} (CUDA-Beschleunigung)" ;;
    amd_rocm)    echo -e "  ${COLOR_GREEN}✓${COLOR_RESET} GPU: AMD (ROCm-Beschleunigung, experimentell)" ;;
    intel_oneapi) echo -e "  ${COLOR_GREEN}✓${COLOR_RESET} GPU: Intel (oneAPI-Beschleunigung, experimentell)" ;;
...
run_diagnose_report() {
  ...
  free_disk_gb=$(df -BG "$check_path" 2>/dev/null | tail -1 | awk '{gsub(/G/,"",$4); print $4}' || echo "unbekannt")
  ...
  gpu_info=$(check_gpu_available) || true
  case "$gpu_info" in
    nvidia:*)    gpu_label="NVIDIA ${gpu_info#nvidia:} (CUDA)" ;;
    amd_rocm)    gpu_label="AMD (ROCm)" ;;
    intel_oneapi) gpu_label="Intel (oneAPI)" ;;
    *)           gpu_label="Keine" ;;
  esac
  ...
  if command_exists journalctl; then
    ...
    log_output="    [Log-Datei nicht gefunden]"
  fi
...
---
Erstellt: ${timestamp}
Script:   setup-ollama-linux.sh v${SCRIPT_VERSION}
...
show_help() {
  cat <<EOF
Hablará Ollama Setup für Linux v${SCRIPT_VERSION}
...
preflight_checks() {
  ...
  case "$gpu_info" in
    ...
    amd_rocm) log_success "AMD GPU erkannt (experimentell)" ;;
    intel_oneapi) log_success "Intel GPU erkannt (experimentell)" ;;
    cpu) log_warn "Keine GPU erkannt - Verarbeitung ohne GPU-Beschleunigung" ;;
  esac
...
start_ollama_server() {
  ...
  if command_exists systemctl; then
    ...
    if systemctl is-active --quiet ollama 2>/dev/null; then
      ...
      log_info "Ollama System-Service aktiv, warte auf API..."
      wait_for_ollama && return 0
    fi
    if systemctl list-unit-files ollama.service 2>/dev/null | grep 'ollama.service' > /dev/null; then
      log_info "Starte Ollama via systemd (System-Service)..."
    ...
    if systemctl --user list-unit-files ollama.service 2>/dev/null | grep 'ollama.service' > /dev/null; then
      log_info "Starte Ollama via systemd (User-Service)..."
  ...
  if command_exists ollama; then
    ...
    log_warn "Ollama Prozess fehlgeschlagen - Log: ${log_file}"
    return 1
  fi
